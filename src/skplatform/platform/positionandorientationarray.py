    ...
    * :meth:`~.clear` to clear the current list of samples in the policy object
    ...
    def to_optical_geometry(self) -> List[OpticalGeometry]:
        ...
        for i in range(self.numsamples_in_observationset()):
            ...
            mjd = sktimeutils.ut_to_mjd(self.utc()[i])
    ...
    #           to_optical_geometry
    # ------------------------------------------------------------------------------
    def to_ecef_position_and_look(self, icf_lookvectors=None, one_to_one: bool=False) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
        ...
        if (numdims != 2) or (s[1] != 3):
            logging.warning("The incoming array of ICF look vectors must be an array of shape (N,3). Your shape is {}".format(s))

        numinstantaneous_los = s[0]
        num_times = self.numsamples_in_observationset()

        if (one_to_one):
            ok = (numinstantaneous_los == num_times)
            if not ok:
                raise ValueError('The one_to_one flag can only be used if the number of ICF look vectors {} equals the number of positions and orientations {} '.format(numinstantaneous_los, num_times))
            numinstantaneous_los = 1

        ecef_observer = np.zeros([num_times, numinstantaneous_los, 3])
        ecef_look = np.zeros([num_times, numinstantaneous_los, 3])
        ut = np.zeros([num_times, numinstantaneous_los], dtype='datetime64[us]')

        for i in range(num_times):
            ecef_observer[i, :, :] = self._position[i].transpose()
            ut[i, :] = sktimeutils.ut_to_datetime64(self._utc[i])
            R = self.icf_to_ecef_rotation_matrix()[i].R

            if (one_to_one):
                ecef_look[i, 0, :] = R @ icf_lookvectors[i, :]
            else:
                for j in range(numinstantaneous_los):
                    ecef_look[i, j, :] = R @ icf_lookvectors[j, :]

        if (one_to_one):
            ecef_observer = ecef_observer.reshape([num_times, 3])
            ecef_look = ecef_look.reshape([num_times, 3])
