...
# ------------------------------------------------------------------------------
ClassicalOrbitalElements = namedtuple('ClassicalOrbitalElements', ('H', 'e', 'RA', 'i', 'W', 'TA', 'a'))
ClassicalOrbitalElements.__doc__ = \
    '''
    The 6 ali_elements of classical orbital ali_elements
...
def coe_from_state_vector(R: np.ndarray, V: np.ndarray) -> ClassicalOrbitalElements:
    '''
    Calculates classical orbital ali_elements (ie Kepler) given a position and velocity. This is taken from
    ...
    a - semimajor axis (km)
    pi - 3.1415926...
    coe - vector of orbital ali_elements [h e RA incl w TA a]
    '''
    ...
    H = np.cross(R, V)                                      # the angular momentum vector (mˆ2/s)
    h = norm(H)                                             # the magnitude of H (mˆ2/s)
    incl = math.acos(H[2] / h)                                # Equation 4.7: inclination of the orbit (rad)
    N = np.cross([0, 0, 1], H)                              # Equation 4.8: the node line vector (kmˆ2/s)
    ...
    E = 1.0 / mu * ((v * v - mu / r) * R - r * vr * V)      # Equation 4.10: eccentricity vector
    e = norm(E)                                             # Calculate eccentricity

    if n != 0:                                              # Equation 4.12 (incorporating the case e = 0)
        if e > eps:                                         # eps - a small number below which the eccentricity is considered to be zero
    ...
    a = h * h / mu / (1.0 - e * e)                           # Equation 2.61, semimajor axis (m),  (a < 0 for a hyperbola)
    coe = ClassicalOrbitalElements(H=h, e=e, RA=RA, i=incl, W=w, TA=TA, a=a)
    return coe
