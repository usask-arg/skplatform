...
def gibbs(pos1, pos2, pos3):
    ...
    if np.abs(np.dot(pos1, c23) / r1 / norm(c23)) > tol:                        # Check that pos1, pos2 and pos3 are coplanar; if not set error flag:
        ierr = 1
        V2 = None
    else:
        ...
        V2 = np.sqrt(mu / norm(N) / norm(D)) * (np.cross(D, pos2) / r2 + S)     # Equation 5.22:
