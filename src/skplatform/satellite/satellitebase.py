...
class SatelliteBase(PlatformLocation):
    ...
    conversion to and from ECI-TEME is provided by the `eci` functions in package `sktimeutils` which have been checked to centimeter
    precision.
    ...
    The `use_polar_motion` flag can be set to True to enable the use of corrections for polar motion when converting ECI to ITRS.
    These corrections are not expected to be required for typical use cases, therefore the default value is False.
    """
    ...
    def __init__(self):
        ...
        self.use_polar_motion: bool = False                         # The flag controlling use of polar motion in the ECI to ITRF conversion
    ...
    def position(self) -> np.ndarray:
        ...
        """
        return sktimeutils.eci.eciteme_to_itrf(self._m_ecilocation, self._m_time, self.use_polar_motion)
    ...
    def velocity(self) -> np.ndarray:
        ...
        Returns:
          np.ndarray[3]
            The three element array specifying (X,Y,Z) in meters
        """
        return sktimeutils.eci.eciteme_to_itrf(self._m_ecivelocity, self._m_time, self.use_polar_motion)
    ...
    def update_position(self, utc: Union[datetime, np.datetime64, float]) -> np.ndarray:
        ...
        """
        ut = sktimeutils.ut_to_datetime(utc)                                          # The satellite code is based upon datetime, so ensure we havethe right time units
    ...
    def update_velocity(self, utc: Union[datetime, np.datetime64, float]) -> Union[np.ndarray, None]:
        ...
        ut = sktimeutils.ut_to_datetime(utc)
        self.update_eci_position(ut)
